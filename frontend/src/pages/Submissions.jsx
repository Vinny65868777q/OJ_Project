...
const Submissions = () => {
  ...
  useEffect(() => {
    const fetchhistory = async () => {
      try {
        const res = await axios.get('http://localhost:5000/api/submission', { withCredentials: true });
      ...
      {submissions.length === 0 ? (
        ...
        <div className="history-table-wrapper">
          <table className="history-table">
            ...
            <tbody>
              {submissions.map(sub => (
                <tr key={sub._id} className={sub.verdict === 'Accepted' ? 'accepted' : 'failed'}style={{ cursor: 'pointer' }}
            onClick={() => navigate(`/submission/${sub._id}`)}
          >
                  <td className="prob-title"> {sub.problemId.title}  </td>
